...
int main(){
    ...
    void*q=&b;
    cout<<p<<endl;
    cout<<q<<endl;//but we cant do *P or *q i,e deferencing with void poiinter
    }*/
...
//the rule is that if we write this manully we also have to write its destructor,copy constuctir,copy assignment,move constructo,move assignment(rule of 3/5)
...
//Function: a finished recipe with fixed types-we write it once and it always takes the same kind of inputs
...
//paramteric polymorphism: "T can be anything"
//contrained parametric poymorphism: "T can be anything that satisfies these rules(concepts)"
//A contraints is an annotation of a template specifying requiremnets on its arguments.These requiremnets are checked by the compiler when instantiating a template; template instantiation will fail if they are violated. constains are boolean expressions evaluaated ta compile time
//A concepts is a named set of requirements concepts define reusable constrains and cna themselves be used  to formulate further constraints
//The keywords requires and constexpr
//requires: is used to add a condition to a template, write a rule that template paramters must satisfy
//constexpr:means can be evalauted at compile time. i,e must be a constant expression(known at "compile time")
/*#include <iostream>
using namespace std;
...
int main(){
    int x= add(16,8);
    //double y= add(4.5,5);//this wont compile and compiler throws the error because we made the requirement as int
    cout<<x<<"\n"<<endl;
}*/

//lecture 13
//let us say that we are doing result=a+b+c c++ takes it as a+b first and save it let say in temp and then it does temp+c and later temp is destroyed this leads to running the loop two times than one time which will be very slow
/*#include<iostream>
using namespace std;
struct Number{
    int value;
...
Number operator+(const Number& x, const Number& y){
    cout<<" adding "<<x.value<<" and "<<y.value<<"\n";
...
int main(){
    vector <int> a={1,2,3};
    vector <int> b={3,6,3,4};
    vector <int> c={2,8,4};
    vector<int>result(b.size());//check with alloting the sapce 2 or 1 but the answer need 3 but for the result[i] the code compiles and runs but with the other uisng .at() will show errro so this is imporatant to make the 
    for(int i=0; i<4; ++i){
        //result[i]=a[i]+b[i]+c[i];
        result.at(i)=a[i]+b[i]+c[i];
...
//we wrote manually the one loop fused version which is indeed fast but it can be unsafe/fragile if we dont handle sizez correctly
//proxy obeject = i dont store numbers i only know how to compute them when someone asks
//the proxy stores references to a , b,c stores the strcture of the computation 
//proxy version or lazy evaluation
/*#include<iostream>
#include<vector>
#include<algorithm>
using namespace std;
struct sumproxy3{
    ...
    const vector<int>& b;
    const vector<int>& c;
    sumproxy3(const vector<int>& a_, const vector<int>& b_,const vector<int>& c_):a(a_), b(b_), c(c_){}
 int size() const {return (int) min({a.size(),b.size(), c.size()});
 }
 int operator[](int i) const {
    return a[i]+b[i]+c[i];
 }
};
sumproxy3 make_expr(const vector<int>&a, const vector<int>& b, const vector<int>& c){
    cout<<"buiding proxy(no loop,no temp vector)\n";
    return sumproxy3(a,b,c);
}
int main(){
    vector<int> a ={2,3,4};
    vector <int> b={3,4,2};
    vector <int> c={3,7,4};
    auto expr = make_expr(a,b,c);
    vector<int> result(expr.size());
    cout<<"now evaluating in one loop:\n";
...
//In C++ we can build objects that describe computations and evaluate them later, whcih is the core idea behind expression templates
//code 1
/*#include <iostream>
#include <vector>
using namespace std;
...
//this above exmaple is not the an exmaple of expression template
/*#include <iostream>
using namespace std;
struct sumexpr{
    int a;
    int b;
    int c;
    sumexpr(int a_, int b_, int c_): a(a_), b(b_), c(c_) {}//constructor with member initialiser list
    int value() const{//computes later hence lazy evaluation as in the lecture slide
        return a+b+c;
    }
...
int main(){
    sumexpr expr(2,3,4);
    cout<<"nothing is being computed yet\n";
    int result = expr.value();
    cout<<"Result="<<result<<endl;
}*/
//we can the above code 1 for big vectors /matrices but its not flexible and doesnt preserve natural math syntax for arbitrary expression hence expression templates are used 
//we can direclt use the Eigen library here whcih i think also uses the concept of expression templates
//syntax of expression template
...
struct VecSum {
    const L& left;//left is the reference to left operand
    const R& right;//right is the reference to right operand
    VecSum(const L&l , const R& r): left(l) , right(r){}//constructor
    int operator[](int i) const{//computed on demand
        return left[i] + right [i];   
         }
};
template <class L, class R>
VecSum<L,R>operator+(const L&l , const R& r) {
    return VecSum<L,R>(l,r);
}
int main(){
    vector<int>a={1,2,3};
    vector<int>b={10,20,30};
    auto expr =a+b;//proxy is created
    cout<<"Accessing expr[i] compute on demand:\n";
    cout<<"expr[0]= "<<expr[0]<<"\n";
    cout<<"expr[1]= "<<expr[1]<<"\n";
    cout<<"expr[2]= "<<expr[2]<<"\n";
...
//lecture 14: latest cpp standard may be c++20 and c++23 are dealt in detail in the slide and giving a glimpse of c++26 without any promise yet beacause features might change 
//1)Concepts: type constraints for templates 2)Modules: A replacement for header files 3)Ranges: Fucntional composition for algorithms 4) Coroutines: Asynchronous programming and lazy generation
//in the template thing we if error occurs due to type it is difficult to get to know the problem and the compiler also thorws a 100 lines of error so before the arrival nof the concept of conepct in cpp20 we were dependent on SFINAE
/*#include <iostream>
#include <concepts>
using namespace std;
template<class t >
...
int main(){
   cout<< add(5,10)<<endl;
   cout<< add(5.5,2.1)<<endl;
   //cout<<add1("a","b")<<endl;
   //cout<<add("a","b")<<endl;
}*/
// requires are also concept of <concpets>
//Module: Initiaally when we write #inlcude"math.hpp" to include some header file what compiler does it it copy paste the entire file and if we have 100 of such inlcudes the that would happen 100 times and hence slowing down the process and hence the conept fon modules were introduced
//what modules does is it compile once and import(fast + clean)
//Range: something we can iterate through(like in a for loop) so range is absically a sequence of elements
/*#include<iostream>
#include <ranges>
#include <vector>
using namespace std;
int main(){
    vector<int> nums={1,2,3,4,5,6,3,2,2,5,7,90};
    auto result = nums |
...
//std::views comes under #include<ranges> and filter ,transform take are words in std::views we cant use another words for this filter, transform, take
//to understand here is an example: sync= we order food and stand at the counter doing nothing untill its ready and Async= we order food, sit down do something else and they call us when its ready
//coroutine as 3 keywords 1)co_wait=wait here then continue later 2)co_yield=give one value now pause and continue later 3)co_return= finish the coroutine
//coroutine we will learn later
/*#include<iostream>
#include<compare>
using namespace std;
struct point{
    int x;
    int y;
    auto operator<=>(const point&) const= default;//we need to define this because c++ has == <>  this built in type(like int , double , pointer) not for what we have here
};
int main(){
    point p1{10,20};
    point p2{10,20};
    if(p1==p2){
        cout<<"this are same"<<endl;
    }
    if(p1>p2){
        cout<<"greater"<<endl;
    }
...
int main(){
    student s ={//we can use the old way but here this gives the flexibility for creating objects
        .roll=20,
        .weight=30,
        .name="abc",
        .male=true
...
#include <span>
using namespace std;
void print_data(std::span<int> data){//std::span is pointer + size together
        cout<<i<<" ";
    }
}
int main(){
    int arr[]={2,3,4,56};
    vector<int> v={4,5,6,7,3,2};
    print_data(arr);
    print_data(v);
}*/
//till now it was all about c++20 we will now start whats polished in c++23 and also we need c++23 compiler
//verbose measn using more words or code than necessary to express something simple
//use of println 
/*#include<iostream>
#include<string>
#include<print>
...
int main(){
    ...
    int id = 40;
    int pass=2345;
    println("{}{}{}", user , id, pass);//this is the syntax for printing this 
    println(stderr,"error happend {}",5);
}*/
/*#include <iostream>
#include <print>
#include <vector>
...
//fucntion often need to return either a value or failure but exceptions are implicit/expensive std::optional doesnt tell why it failed and error code separate error from data, C++23 std::expected<T,E> returns either a succesfull value T or an error E in one type, so we can handle failures explicity with the reason
/*#include<iostream>
#include <expected>
using namespace std;
...
//enum means a set fi named choices, exmaple enum class light{red,yellow, greem}; this mean a varible of type light cna be only one of these three
//we will use the above code exmaple with std::expetced and enum
/*#include <iostream>
#include <expected>
using namespace std;
enum class diverror{Dividebyzero};
std::expected<double , diverror>
safe_div(double a, double b){
    if(b==0){
...
int main(){
    double a,b;
    cin>>a>>b;
    auto res = safe_div(a,b);
    if(res){
        cout<<*res<<"\n";
    }else{
        if(res.error()==diverror::Dividebyzero)//.error() is a member function of std::expected
        cout<<"error divide by zero"<<endl;
    }
}*/
//we keep the data in 1d array std::mdspan wraps it and give us in matrix style access
/*#include <vector>
#include <mdspan>
#include <print>
...
//with multidimensional in c++23 we can now write m[row,col]for our own matrix type
...
class vec{
    ...
    vector<T>add(const vector<T>&x,const vector<T>&y){
        if(x.size()!=y.size())
        throw runtime_error("vector must have same size");
        vector<T> z(x.size());
...
int main(){
    ...
    vec<int> s;
    vector<int>z=s.add(x,y);
    for(const auto& v:z)cout<<v<<" ";
    cout<<'\n';
}*/
/*#include<iostream>
#include<vector>
...
class Vec{
    ...
    vector<T>add(const vector<T>& x, const vector<T>& y){
        if(x.size()!=y.size())
        throw runtime_error("vector must be of same size");
        vector<T>z(x.size());
        for(size_t i=0; i<x.size(); ++i)
        z[i]=x[i]+y[i];
        return z;
...
int main(){
    vector<int>x={2,23,4,56};
    vector<int>y={3,4,2,4};
    Vec s;
    auto z=s.add(x,y);
    for(auto a:z)
    cout<<a<<endl;
...
//size type 
//size_t: an unsigned integer type big enopugh to represent the size of the largest possible object in memory 
/*#include <iostream>
#include<cstddef>
using namespace std;
int main(){
